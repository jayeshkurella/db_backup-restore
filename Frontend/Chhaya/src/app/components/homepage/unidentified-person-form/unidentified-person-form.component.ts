...
export class UnidentifiedPersonFormComponent implements OnInit {
  ...
  constructor(private fb : FormBuilder , private UPservice :UnidentifiedpersonformApiService){}
  ...
  ngOnInit(): void {
    ...
    this.UnidentifiedPersonForm = this.fb.group({
      ...
      availability_for_search_operations: [''],
      preferred_mode_of_communication: [''],
      access_to_vehicle: [false],
      special_skills: [''],
  ...
  private updateFormCoordinates(latitude: number, longitude: number): void {
    const control = this.UnidentifiedPersonForm.get("address.location");
    if (control) {
  ...
  onFileChange(event: any, type: string): void {
    ...
    if (file) {
      if (type === 'photo') {
        this.UnidentifiedPersonForm.patchValue({ photo_upload: file });
      } else if (type === 'file') {
        this.UnidentifiedPersonForm.patchValue({ fir_photo: file });
      }
    }
  }
  ...
  submitupForm() {
      const formValues = this.UnidentifiedPersonForm.value;
      ...
      this.UPservice.postMissingPerson(payload).subscribe({
        next: (response) => {
