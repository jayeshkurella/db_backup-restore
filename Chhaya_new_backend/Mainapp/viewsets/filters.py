...
class filter_Address_ViewSet(viewsets.ViewSet):
    ...
    def districts(self, request):
        ...
        districts = Address.objects.filter(state=state).values_list('district', flat=True).distinct()
    ...
    def cities(self, request):
        ...
        cities = Address.objects.filter(district=district).values_list('city', flat=True).distinct()
    ...
    def villages(self, request):
        ...
        villages = Address.objects.filter(city=city).values_list('village', flat=True).distinct()
